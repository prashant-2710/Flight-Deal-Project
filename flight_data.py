...
def find_cheapest_flight(data, return_date):
    ...
    #Combine all flights best and others
    ...
    initial_stops = len(first_flight["flights"]) -1
    cheapest_flight = FlightData(lowest_price, origin, destination, out_date, return_date, stops = initial_stops)

    for flight in all_flights:
        try:
            price = flight["price"]
        except KeyError:
            print("--- No Flight Price available. ---")
            continue
        ...
        if price < lowest_price:
            ...
            print(f"Lowest price to {destination} in GBP {lowest_price} ({nr_stops} stops)")
